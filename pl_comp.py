...
def pl_comp_scope(fenv: Func, node):
    ...
    for g in groups:
        ...
        funcs = [
            pl_scan_func(fenv, kid) for kid in g if kid[0] == "def" and len(kid) == 4
        ]
        # compile subexpressions
        for kid in g:
...
class CodeGen:
    ...
    def create_stack(self, data):
        ...
        self.buf.extend(
            ...
            b"\x0f\x05" % operand(data)  # syscall
        )
    ...
    def binop(self, op, a1, a2, dst):
        ...
        if op in ("/", "%"):
            ...
            self.buf.extend(
                b"\x48\x85\xc0" b"\x0f\x95\xc0"  # test rax, rax  # setne al
            )
            # mov rdx, [rbx + a2*8]
            ...
            self.buf.extend(
                b"\x0f\x95\xc0" b"\x0f\xb6\xc0"  # setne al  # movzx eax, al
            )
        else:
            raise NotImplementedError
    ...
    def jmpf(self, a1, L):
        self.load_rax(a1)
        self.buf.extend(b"\x48\x85\xc0" b"\x0f\x84")  # test rax, rax  # je
...
def test_comp():
    ...
    assert (
        asm("(var a 1) (set a (+ 3 a)) (var b 2) (- b a)").split()
        == """
        func0:
            const 1 0
            const 3 1
...
def main():
    ...
    ap.add_argument(
        "--exec", action="store_true", help="compile to memory and execute it"
    )
    ap.add_argument("-o", "--output", help="the output path")
    ap.add_argument(
        "--print-ir", action="store_true", help="print the intermediate representation"
    )
    ...
    if args.exec:
        ...
        with open("./code_output", "w") as file:
            hex_string = "\n".join(f"{byte:02x}" for byte in gen.buf)
